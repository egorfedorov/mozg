> **Историческая записка.** Писалось до перехода в опенсорс (2026-08-05):
> цены и «платные мозги» ниже — прошлая модель. Актуальная экономика:
> открытый код (AGPL), бесплатный каталог, деньги — за облачный инференс
> (планы) либо BYOK. См. MARKETING.md.
...
## 17. Что дальше — аудит и волна улучшений

*Дописано 2026-08-04; первая волна реализации закрыта в тот же день — 23 идеи из 30.* После запуска продукт прошёл аудит по живому продакшену — 30 идей, сгруппированных ниже. Это меню, а не обязательства: отбор и порядок — в [TASKS.md](./TASKS.md). Часть пунктов упирается во внешние ключи, которые может выдать только владелец проекта; такие помечены честно, чтобы не путать «не сделано» с «заблокировано».

### Уроки и обучение людей (learn)

Learn-курсы компилируются из тех же заметок и проверок, что обслуживают агентов, поэтому большинство идей здесь дёшевы: это второй вид на уже существующие данные, а не новый контур.

1. ~~**Уроки из вопросов экзамена**~~ — сделано: секции урока привязаны к `check_ids` экзамена, структура курса повторяет структуру checks.
2. ~~**Адаптивные уроки по пробелам**~~ — сделано: `rankSections`, слабые секции идут первыми (weak first).
3. ~~**Интервальное повторение секций**~~ — сделано: SM-2 для целых секций (миграция `0031`), review-очередь секций.
4. **Runnable-примеры.** Автовставка в уроки кода из заметок `kind=example`. *Не брались.*
5. ~~**Мини-квиз после каждой секции**~~ — сделано вместе с п. 1: квиз после каждой секции в плеере.
6. ~~**Режимы глубины**~~ — сделано: ELI5 / standard / expert в payload урока, переключатель в плеере.
7. ~~**Дуэль с агентом**~~ — сделано: achievements (миграция `0044`), бейдж «You beat your agent».
8. ~~**Публичный бейдж «агент сдал экзамен на N%»**~~ — сделано: публичный `/b/[handle]/[slug]/badge` с OG-мета и anti-bluff метрикой.
9. **Аудио-уроки.** TTS поверх скомпилированного intro + sections. *Открыто: нужна TTS-инфраструктура.*
10. ~~**Learning path**~~ — сделано: path семьи на странице курса с прогресс-баром.

### Умность мозгов

11. ~~**Экзамен из реальных провалов поиска**~~ — сделано: search-gap harvest в maintenance, `origin='search_gap'` (миграция `0042`).
12. ~~**Авто-дозалив пробелов**~~ — сделано: `gap_suggestions` (миграция `0043`), UI на странице мозга.
13. ~~**Кросс-экзамен версий**~~ — сделано: мини-экзамен после реинжеста, `exam_regressions` (миграция `0047`), тег «went stale».
14. ~~**Веса заметок по использованию**~~ — сделано: `notes.weight` (миграция `0035`), `brain_feedback` useful up/down.
15. **Включить консолидацию.** *Открыто: `CONSOLIDATE_ENABLED` всё ещё false, ждёт валидации порога.* O(n²) дедуп из этого пункта при этом уже убран — переписан на kNN, так что пункт сузился до самого включения.
16. ~~**Иерархическая память**~~ — сделано: таблица `summaries` (миграция `0041`), `brain_brief` включает summary первыми — закрывает открытый вопрос §16.4 про мозг на 200 источников.
17. ~~**Экзамен на «отрицательные знания»**~~ — сделано: `checks.kind` negative (миграция `0036`), anti-bluff метрика.
18. ~~**Дифф-экзамен при покупке**~~ — сделано: блок «answers N/M · anti-bluff X/Y» на публичной странице мозга.
19. ~~**Коллективный мозг**~~ — сделано: `/api/collective` + рабочая форма на `/collective`.

### Продукт и деньги

20. ~~**Checkout тарифов**~~ — сделано: `plan_requests` + оплата с баланса + `paid_until` (миграция `0040`), админка approve/reject.
21. **Регистрация по email.** Код готов, письма подтверждения и сброса написаны в `lib/auth.ts` и включаются сами. **Блокер:** только ключи `RESEND_API_KEY` + `EMAIL_FROM` (см. TASKS.md, п. 3).
22. **Google OAuth** как второй провайдер рядом с GitHub. *Открыто: нужны ключи.*
23. **Заполнить каталог.** Запустить заготовку `scripts/catalogue.ts` на проде; цель — 20 публичных мозгов (см. TASKS.md, п. 5).
24. ~~**Аналитика воронки PostHog**~~ — сделано: подключён, события `signup` → `first_brain_search` и др.
25. **Лид-магнит.** Один polished бесплатный learn-курс для привлечения. *Открыто: контентная задача.*

### Техника

26. ~~**CI (GitHub Actions)**~~ — сделано: `.github/workflows/ci.yml`, `tsc` + `npm test` на каждый пуш.
27. ~~**Тесты на деньги и доступ**~~ — сделано: `payments.ts`, `access.ts`, `tokens.ts` покрыты, 125 → 176 тестов.
28. ~~**Глубина подтверждений крипто-сеттлемента**~~ — сделано: `MOZGPAY_*_CONFIRMATIONS`. Проверка на живых деньгах всё ещё ждёт ключей `MOZGPAY_*`.
29. ~~**Собранный воркер в проде**~~ — сделано: `build:worker`, Dockerfile с `--omit=dev`.
30. **Lazy-долг в задачи** — частично: исправлены SSE-гонка, O(n²) дедуп (kNN), добавлены `limit_req_zone` в nginx, почта оператора в `OPERATOR_EMAIL`. Открыты: html→text (`src/lib/page.ts:85`), стемминг tsvector (`src/db/migrations/0001_app.sql:125`), подсчёт токенов (`src/lib/chunk.ts:13`).

Отдельно в этой же волне — фикс HMAC-каноникализации платёжной подписи (`src/lib/payments.ts`): `canonicalJson` стала рекурсивной и байт-совместимой с плоскими payload, так что старые подписи не сломались.

**Важно про деплой:** миграции `0031`, `0035`, `0036`, `0040`–`0044`, `0047` применены только к локальной БД — на прод волна не выкатывалась. Следующий деплой обязан прогнать миграции. nginx-конфиг изменён: на сервере проверить дубликат `limit_req_zone` — см. комментарий в `deploy/nginx-mozg.sh.conf`.
