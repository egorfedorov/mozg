...
| `src/lib/crawl.ts` | одна ссылка → все страницы доков: GitHub-дерево, llms.txt, sitemap, обход ссылок; только новейшая версия; JS-shell → его репозиторий (по ссылке в HTML или карте известных сайтов) |
| `src/worker/crawl.ts` | разворачивает site-источник в url-источники (повтор ничего не дублирует), у мозга без цели — генерирует цель из материала |
| `src/lib/mail.ts` | отправка почты через Resend REST; всё, что зависит от почты, включается ключами |
...
| `/settings/topup` | пополнение: крипта (NOWPayments или ручной запрос), карта и прочее — «скоро» |
...
регистрация по паролю закрыта; письма подтверждения и сброса пароля уже
написаны в `lib/auth.ts`) и крипто-пополнение (`NOWPAYMENTS_*` — без них
страница `/settings/topup` предлагает ручной путь). Код готов и включается сам.
...
| `npm run requeue -- --brain <slug>` / `--failed` / `--reread` | вернуть источники в очередь; `--reread` перечитывает и готовые с текущим промптом извлечения |
