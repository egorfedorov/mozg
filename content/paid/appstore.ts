export const NOTES: {
  ...
  {
    title: "Rejected under 4.2 Minimum Functionality — what it means and the actual fix",
    body: "4.2 targets apps that are little more than a website wrapper, a marketing brochure, or a thin utility the reviewer finishes in 30 seconds. The related 4.2.2 clause covers web-clipping reskins and 'app makers' output — templated containers around someone else's content. What does NOT fix a 4.2 rejection: adding a login wall, adding push notifications, or adding a settings screen — reviewers treat those as decoration on the same thin core. What does: native functionality that justifies the app existing — offline capability, device APIs the website can't touch (camera, sensors, widgets, CarPlay/watch), native navigation and performance, or content depth the reviewer can actually explore. In the appeal or resubmission notes, enumerate the native features explicitly; reviewers reject again when the reply argues policy instead of listing functionality.",
    category: "Apple guideline clauses",
    kind: "rule",
  },
  {
    title: "What is guideline 2.3.1 and how do reviewers actually detect hidden features?",
    body: "2.3.1 prohibits hidden, dormant, or undocumented features — including feature flags, remote-config toggles that change behavior after approval, and code paths not disclosed in review notes. Detection is not a human tapping around: Apple runs static analysis on the binary looking for undisclosed private API usage, unused-but-present capability strings, and symbols referencing functionality not in the review notes; dynamic analysis observes network calls and behavior differences between review conditions and normal use; and post-approval re-review sweeps re-check flagged apps. The practical rules: strip dead code and unused flags before submission (they read as hidden features), declare every remotely-toggleable behavior in review notes, and never ship a flag that materially changes what the reviewed app does.",
    category: "Apple guideline clauses",
    kind: "pitfall",
  },
  {
    title: "What is the current Google Play target API deadline, and what happens if I miss it?",
    body: "Play enforces an annual target-API cycle with an August 31 deadline: new apps and app updates must target the API level from the prior year's Android release (the 2025 cycle required targetSdk 35 / Android 15; the 2026 cycle moves to API 36 — verify exact dates as of early 2026, Play announces them each spring). Missing it has two different consequences: new apps and updates are rejected at upload (you simply cannot submit), and EXISTING apps below the threshold lose distribution — they become undiscoverable and unavailable for new installs on devices running newer Android, though existing users keep the app. Play usually grants an extension request window (historically to around November) via Play Console. Treat August as a hard calendar event: schedule the targetSdk bump and regression pass for June–July, not the last week of August.",
    category: "Google Play specifics",
    kind: "fact",
  },
  {
    title: "What are the hard rules for App Store screenshot format and content?",
  ...
  {
    title: "My second app shares a codebase with the first and got 4.3 Spam — what are the remedies?",
  ...
  {
    title: "What does 2.1 App Completeness actually cover, and what triggers it?",
    body: "2.1 requires the app to be complete and functional at review time. The recurring triggers: no demo/test account provided for login-required apps (put working credentials in the App Review notes field — reviewers will not register); placeholder or broken content (dead buttons, 'TODO' screens, lorem ipsum, empty feeds); crashes on launch or on the primary flow — reviewers run on real current hardware; backend not live or pointing at staging/localhost during review; required hardware not available to the reviewer (if the app needs a Bluetooth accessory, say so and provide a video or demo mode); and IAP products not submitted together with the build. The pre-submission drill: fresh device, fresh install, airplane-mode-once, every button tapped, with the exact credentials you pasted into review notes.",
    category: "Apple guideline clauses",
    kind: "rule",
  },
  {
    title: "Can I link from my paywall to buy on my website — Apple AND Google Play?",
    body: "Apple: digital goods must use IAP (3.1.1), and an unauthorized external purchase link is a 3.1.1 rejection. The exceptions: reader apps (3.1.3(a)) with the External Link Account entitlement may link to account management/purchase on the web; in the US storefront the StoreKit External Purchase Link entitlement allows a link-out under its specific rules, and post-Epic injunction US apps may link to external purchase without the commission — this area is actively changing as of early 2026, verify the current entitlement terms. Google Play: the Payments policy requires Google Play billing for digital goods, but Play runs User Choice Billing (user picks Play billing or the developer's own billing at checkout, with a reduced service fee) and an alternative-offers program in some regions — enrollment is required, you cannot just add your own checkout link. In both stores an unapproved external checkout link is one of the fastest rejections there is.",
    category: "Payments and IAP",
    kind: "rule",
  },
  {
    title: "I'm shipping a fintech/crypto app — what documentation do reviewers want?",
    body: "Prepare before submitting, not after the rejection: proof of licensing/registration for every operating region (money transmitter, EMI, broker, or gambling licenses as applicable); for crypto specifically (Apple 3.1.5) — wallets are fine, but crypto EXCHANGE apps must be submitted by the exchange itself, under the exchange's own developer account, and only by established/licensed exchanges; no on-device mining is allowed; NFT features that unlock in-app functionality must go through IAP. Gambling apps (5.3) must be free on the store, licensed for every served territory, and geo-restricted. Upload the license documents proactively in the review notes attachment — the default reviewer action for an undocumented finance app is rejection, and proving licensure afterwards costs a week per round-trip.",
    category: "Special categories",
    kind: "rule",
  },
  {
